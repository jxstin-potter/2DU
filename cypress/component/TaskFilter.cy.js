...
var TaskFilter_1 = __importDefault(require("../../src/components/TaskFilter"));
describe('TaskFilter Component', function () {
    ...
    beforeEach(function () {
        cy.mount((0, jsx_runtime_1.jsx)(TaskFilter_1.default, { onFilterChange: mockOnFilterChange, availableTags: mockTags }));
    });
    it('renders filter options correctly', function () {
    ...
    it('handles status filter changes', function () {
        // Select active filter
        cy.get('[data-testid="status-filter"]').click();
        cy.get('[data-testid="status-option-active"]').click();
        // Verify onFilterChange was called with correct status
        cy.get('@onFilterChange').should('have.been.calledWith({, status, 'active', tags, []);
    });
});
it('handles tag filter changes', function () {
    ...
    cy.get('@onFilterChange').should('have.been.calledWith({, status, 'all', tags, ['Work', 'Personal']);
});
;
it('handles combined status and tag filters', function () {
    ...
    cy.get('@onFilterChange').should('have.been.calledWith({, status, 'completed', tags, ['Work']);
});
;
it('clears all filters', function () {
    ...
    cy.get('@onFilterChange').should('have.been.calledWith({, status, 'all', tags, []);
});
;
it('displays active filter count', function () {
...
it('handles tag search', function () {
    // Type in tag search
...
it('maintains filter state', function () {
    ...
    cy.mount((0, jsx_runtime_1.jsx)(TaskFilter_1.default, { onFilterChange: mockOnFilterChange, availableTags: mockTags, initialStatus: "completed", initialTags: ['Work'] }));
